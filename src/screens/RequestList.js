...
const DonerList = props => {
  ...
  const ApiDonor = async () => {
    ...
    console.log('user id on request list = ', IdUser);

    let url = `https://bloodlinks.in/myrequestlist?user_id=${IdUser}`;
    var headers = {
    ...
    fetch(url, {
      ...
      .then(Response => {
        ...
        console.log('response of request list = ' + Response);

        setrequestlist(Response);
        setrequest_id(Response[0].id);
        //setrequest_id(Response[0].IdUser)
      })
      .catch(error => {
  ...
  const Confirm = () => {
    if (date1 != null && Reason != null) {
      ApiModal();
      setModalVisible(!modalVisible);
      setIsLoading(true);
    } else {
      alert('Please select date & reschedule reason');
    }
  };

  const ApiModal = () => {
    let url = `https://bloodlinks.in/Re_schedulerequest`; //API to
    var headers = {
      Accept: 'application/json',
      'Content-Type': 'application/json',
    };
    fetch(url, {
      ...
      .then(Response => {
        console.log(
          'RESPONSE apiiii book appointment-------------->>>>',
          Response,
        );
  ...
  return (
    <View style={{flex: 1}}>
      <DrawerHeader name={'Blood Requests History'} image1={false} />
      <View
        style={{
          flex: 1,
          flexDirection: 'column',
