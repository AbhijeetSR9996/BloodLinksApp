...
const DonerList = props => {
  ...
  const ApiDonor = async () => {
    ...
    fetch(url, {
      ...
      .then(Response => {
        console.log(
          'RESPONSE apiiii my appointment list-------------->>>>',
          Response,
        );
        setIsLoading(false);
  ...
  const Confirm = () => {
    if (date1 != null && Reason != null) {
      ...
      alert('Please first select the appointment date & reason');
    }
  };
  const ApiModal = () => {
  ...
  return (
    <View style={{flex: 1}}>
      <DrawerHeader name={'Donation History'} image1={false} />
      <View
        style={{
          flex: 1,
          flexDirection: 'column',
